...
## Status log
- 2026-09-05: V3-7 Highlight reel + character polish merged (v3 complete): goals, big hits, big saves, ankle breakers and fight KOs are captured as ~2 s clips from the replay ring (last 130 frames, finalized 28 frames after the event); after the player-of-the-game orbit the reel plays up to six clips (goals first) at 0.6× with per-kind cameras and a HIGHLIGHTS n/N tag, any button skips the whole reel; off under reduced motion. Stick-hand IK: the top hand solves a two-bone chain onto its grip point on the stick each frame (two passes, minimal-twist aim), the bottom hand owns the stick, so wind-ups and toe-drags now move the shoulder instead of the stick bone; residual ≤ 4 cm at full wind-up. Goalie mask: team-colored shell with a front opening, bar cage, chin guard (goalie 5900 tris).
- 2026-09-05: V3-6 Accessibility pass merged: Controls screen rebinds any keyboard action (press-to-capture, one key per action, reset), bindings persist in meta and every in-game prompt and training drill fills `{action}` tokens from the live map; colorblind palettes (deuteranopia/protanopia/tritanopia) swap the away jersey to a safe alternate when the two team colors collide under a simulated-vision distance check and recolor pass lanes to blue/orange; fight cues gained shape glyphs (▲▼◆●) so no cue is color-only; name tags above skaters (all / controlled / off); HUD text size 100/125/150%; one reduced-motion switch (no shake, flashes, hit fx, crowd animation, shortened light show); gamepad rumble on big hits and goals (toggle).
- 2026-09-05: V3-5 Feats, weekly seed, records merged: 20 feats (match, run and career triggers) paying bank cash, three of them granting the perk-pool unlocks that were previously purchase-only (Deadeye for a championship, Zamboni Crew for a completed set, Gasoline for team fire); toast on award. Weekly Run: seed `weekly-<ISO week>` (local-time week roll), ascension off, best result per week kept. Feats & Records screen: career totals, best run/match records, rival W-L table, and local run telemetry (perk offer/pick rates, node choices, run end acts) for tuning. Outcome now carries fights won, specials used, ankle breakers, big saves, shootout result, top-corner goals, team fire.
- 2026-09-05: V3-4 Rival grudges + content pack merged: beaten rivals are remembered (`run.grudges`), rematches come a tier higher (cap +2) with a taunt on the intro and a +50%/grudge bounty; when a new act opens, half its match nodes are re-assigned to rivals you already beat. +4 rivals (Ironworks Anvils, Cyclone Jets, Bayou Gators, Graveyard Ghouls), +3 alt bosses (Iron Maidens, Carnival Freaks, The Dynasty) with one boss picked per act at map gen, +4 mutators (Meter Locked, Fight Night, Heavy Puck, Outnumbered), +6 events (Fan Favorite, Equipment Sale, Super Fan bet, Extra Ice Time, The Scout, Old Grudge), +2 archetypes with specials (Grinder BULLDOZE: 5 s unknockable + huge hits; Dangler PHANTOM: 4 s every check whiffs and breaks ankles) and 2 unlockable captains for them. Totals: 15 rivals, 6 bosses, 12 mutators, 16 events, 6 skater archetypes, 7 captains.
