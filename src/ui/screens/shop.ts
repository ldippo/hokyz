...
import { completeNode, draftPerks, runEffects, runRng, commitRng } from '../../run/runState';
import { PERK_PRICE } from '../../run/perks';
import { generateSkater, randomArchetype, ARCHETYPES } from '../../run/roster';
...
export function shopScreen(app: App, node: MapNode): void {
  ...
  const disc = 1 - Math.min(0.6, eff.shopDiscount);
  const price = (base: number) => Math.round(base * disc);
  let perks = draftPerks(run, 3, 0.3);
  ...
  const render = () => {
    ...
    const rerollCost = price(30 + rerolls * 15);
    ...
    const body = h('div', { class: 'map-scroll' },
      ...
      h('div', { class: 'cards' },
        ...
        hired ? null : h('div', { class: `card ${run.cash >= hireCost ? '' : 'disabled'}`, 'data-nav': run.cash >= hireCost ? '1' : '', onClick: () => { if (run.cash < hireCost) return; run.cash -= hireCost; run.roster.push(hire); hired = true; sfx.cash(); app.saveRun(); render(); } },
      ...
      h('div', { class: 'menu', style: 'flex-direction:row;justify-content:center' },
        btn(`Reroll perks (${rerollCost})`, () => { if (run.cash < rerollCost) return; run.cash -= rerollCost; rerolls++; perks = draftPerks(run, 3, 0.3); app.saveRun(); render(); }),
