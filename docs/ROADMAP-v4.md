...
## Status log
- 2026-09-05: V4-5 Overtime League + ascension 3-5 merged: clearing the last act no longer ends the run; `completeNode` marks it won and raises `leagueOffer`, the map routes to a new offer screen (bank the trophy → run over, or keep skating → `extendRun` bolts on Act N+1 via `generateAct` at a higher tier: any boss can appear, tier cap 6 in league acts, +25% cash per league act, one extra boss phase labelled OVERTIME RULES). Losing in the league ends the run with the trophy kept; run-over pays +150 per league act, records track deepest league act and best ascension won. Ascension 3 (drafts always carry a cursed perk, the epic slot goes cursed), 4 (rest stops train only), 5 (every boss gets an extra phase, stacking with the league) as unlockables at 1200/1600/2000 with +15% cash each; `ascensionLevel` reads the highest owned. Feats: Overtime League (clear Act 4), Cursed Crown (win at Asc 3+). Migration fills `league`/`leagueOffer`. Batch unchanged (no sim edits).
- 2026-09-05: V4-4 Quality of life merged: pause menu shows the live box score (G/A/SOG/HITS/BIG/SV-BLK for everyone dressed) and the run's perks, with Photo Mode (`App.photo()` re-renders one frame, composites a scoreboard strip in team colours and a HOKYZ watermark on a 2D copy of the canvas, `snap()` downloads the PNG; verified 677 KB PNG headless). Quick Match gains a "Your team" row: pick-up squad or any unlocked captain with two tier-1 generated linemates and a goalie (`run/quick.ts`). `/?seed=<text>` opens captain select with the seed prefilled; the map's SEED chip and the run-over screen copy that link (`run/share.ts`, clipboard with a textarea fallback), and run-over adds "Replay This Seed". Captain select adds a GOALIE row (Random / Butterfly / Stand-up / Puck-handler) that pins the starting goalie's style (weekly runs stay random). Training Camp gains SHOT BLOCK: stand in the lane marker, block three shots from a scripted shooter (end-to-end test drives the drill to completion). 103 tests.
